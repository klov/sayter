...
$(date).find("group").each(function(index, element) {
    ...
    var html='<div class="konteyner" id="group'+$(this).find("id").text()+'"><span class="lol">'+$(this).find("name").text()+'</span><img class="button" src="img/amblem.gif" onClick="connect()" title="присоеденится"</div>';
		...
		var uid=$("#user"+kl);
		if(!uid.length){
    var html='<div class="konteyner" id="user'+$(this).find("id").text()+'"><a  href="#"class="lol">'+$(this).find("name").text()+'</a><div><img class="button" src="img/amblem.gif" title="присоеденится" onClick="conact.connect_p2p({user_id:\''+$(this).find("id").text()+'\',url:\''+$(this).find("URL").text()+'\',user_name:\'dfg.cpp\'})"><span class=" ui-icon-locked ui-icon button" title="игнорировать"  onClick="ignor(this)"></span><span class=" ui-icon-arrow-4-diag ui-icon button" onClick="clos(this)" title="закрыть"></span></div></div>';
